...
public sealed class NotesController(AppDbContext db) : ControllerBase
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!await IsMember(teamId, userId.Value, cancellationToken))
            return Forbid();
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (role < TeamRole.Editor)
            return StatusCode(403, "Editor access is required.");
        ...
        CancellationToken cancellationToken)
    {
        ...
        var note = await db.Notes
            ...
            .SingleOrDefaultAsync(
                item => item.Id == noteId && !item.IsDeleted,
                cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (role < TeamRole.TeamLead)
            return StatusCode(403, "Team Lead access is required.");
    ...
    private Task<bool> IsMember(
        int teamId,
        int userId,
        CancellationToken cancellationToken)
    {
        return db.TeamMembers.AnyAsync(
            member =>
